...
export class MatchEngine {
  ...
  // Phase 3: Tactical Modifiers
  private readonly tacticalModifiers = {
    mentality: {
    ...
    formation: {
      ...
      '5-0': { offensive: 1.35, defensive: 0.50 } // Fly-goalkeeper: high offense, weak defense
    },
    // Fly-goalkeeper modifiers
    flyGoalkeeper: {
      possession: 0.3,        // +30% possession probability (extra player in attack)
      shotFreq: 0.25,          // +25% shot frequency (numerical advantage)
      counterVulnerability: 0.50, // +50% opponent counter-attack goal probability (GK out of position)
      defensiveWeakness: -0.40    // -40% defensive resistance (no GK)
    }
  };
    ...
    switch (usage) {
      ...
      case 'Sometimes':
        ...
        if (minute >= 30 && scoreDiff < 0) {
          return Math.random() < 0.20; // 20% chance when losing
        }
        if (minute >= 35 && scoreDiff === 0) {
          return Math.random() < 0.10; // 10% chance when drawing late
        }
        if (opposingTactics.pressingIntensity === 'VeryHigh') {
          return Math.random() < 0.15; // 15% chance if opponent is pressing very high
        }
        return false;
  ...
  private updatePossession(state: LiveMatchState): void {
    ...
    if (homeFlyGK) {
      adjustedChance += this.tacticalModifiers.flyGoalkeeper.possession;
    }
    if (awayFlyGK) {
      adjustedChance -= this.tacticalModifiers.flyGoalkeeper.possession;
    }
  ...
  private getTacticalModifiers(
    ...
    const mentalityMods = this.tacticalModifiers.mentality[mentality as keyof typeof this.tacticalModifiers.mentality];
    const pressingMods = this.tacticalModifiers.pressing[pressing as keyof typeof this.tacticalModifiers.pressing];
    ...
    if (this.isFlyGoalkeeperActive(team, state)) {
      shotFreqMultiplier += this.tacticalModifiers.flyGoalkeeper.shotFreq;
      defense += this.tacticalModifiers.flyGoalkeeper.defensiveWeakness;
    }
  ...
  private getFormationModifiers(formation: string): { offensive: number; defensive: number } {
    const formationKey = formation as keyof typeof this.tacticalModifiers.formation;
    return this.tacticalModifiers.formation[formationKey] || { offensive: 1.0, defensive: 1.0 };
  }
  ...
  private generateShotEvent(state: LiveMatchState, isCounter: boolean): void {
    ...
    if (isOnTarget) {
      ...
      if (goalkeeper) {
        const saveProb = 0.20 * (goalkeeper.player.attributes.reflexes || 20) / 20;  // Further reduced from 0.25
        if (Math.random() < saveProb) {
          // Saved!
          state.statistics.saves[defendingTeam]++;
          goalkeeper.performance.rating += 0.1; // Small bonus for save
          shooter.performance.rating -= 0.05; // Small penalty for not scoring
          state.events.push({
            minute: state.currentMinute,
            type: 'shot',
      ...
      // Check for goal (adjusted for futsal's high-scoring nature)
      let goalProb = 0.70 * quality;  // Increased to 0.70 for better conversion
      ...
      if (isCounter && this.isFlyGoalkeeperActive(defendingTeam, state)) {
        goalProb *= (1 + this.tacticalModifiers.flyGoalkeeper.counterVulnerability);
      }
      ...
      if (Math.random() < goalProb) {
        ...
        defendingLineup.forEach(p => {
          p.performance.rating -= 0.15; // Team penalty for goal conceded
        });
        
        // Extra penalty for goalkeeper who failed to save
        if (goalkeeper) {
          goalkeeper.performance.rating -= 0.2; // Additional penalty for GK
        }
  ...
  private selectPlayerForAction(
    ...
    // If fly-GK is active, all 6 players (including GK) are available
